...
const paymentVerification = async (req, res) => {
  ...
  if (isAuthentic) {
    try {
      await Payment.create({ userId, amount, razorpay_order_id, razorpay_payment_id, razorpay_signature });
      res.redirect(`https://expert-consultation-platform.vercel.app/user/paymentsuccess?reference=${razorpay_payment_id}`);
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
  } else {
...
app.post('/expert', async (req, res) => {
  const { username, email, password, categories, price, availability, contact, languages } = req.body;
  const expert = new Expert({ username, email, password, categories, price, availability, contact, languages });
  try {
    await expert.save();
    res.status(201).json({ message: 'Expert created' });
  } catch (err) {
...
io.on("connection", (socket) => {
  ...
  // Only allow file uploads if user is authenticated
  socket.on("send_file", (data) => {
    const { room, author, fileName, fileUrl } = data;
    const newFile = new File({ room, author, fileName, fileUrl, date: new Date() });
    newFile.save()
      .then(() => {
        socket.to(room).emit("receive_file", data);
      })
      .catch(err => console.error(err));
  });
