...
const ExpertUpcomingAppointmentsPage = () => {
  ...
  async function fetchExpertId() {
    try {
      const response = await axios.get(`https://expert-consultation-platform.onrender.com/getExpertData?email=${userdata.email}`);
  ...
  async function fetchBookedAppointments() {
    try {
      ...
      if (expertId) {
        ...
        const response = await axios.get(`https://expert-consultation-platform.onrender.com/expertbooked-appointments?userId=${expertId}`);
