...
function LoginForm({ userType, toggleMode, leftPanel, rightPanel }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      response = await axios.post(`https://expert-consultation-platform.onrender.com/${userType === 'User' ? 'user' : 'expert'}/login`, {
        email,
        password,
      });
      ...
      if (response.status === 200) {
        ...
        navigate(userType === 'User' ? '/user/userprofile' : '/expert/expertprofile', { state: { email } });
      } else {
